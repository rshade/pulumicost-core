...
PulumiCost is designed with security in mind, especially when handling sensitive cloud credentials
and cost data.

## Credential Handling

PulumiCost does **not** store cloud provider credentials. It relies on the environment's configuration
...
- **Plugins**: Plugins run as separate processes and inherit the environment variables of the host
  process. Ensure strict control over the environment where PulumiCost runs.
...
the [GitHub Security Advisory](https://github.com/rshade/pulumicost-core/security/advisories)
