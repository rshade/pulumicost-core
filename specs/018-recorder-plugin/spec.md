...
**Feature Branch**: `018-recorder-plugin`
**Created**: 2025-12-11
**Status**: Draft
...
As a plugin developer, I want to capture and inspect the exact gRPC requests that FinFocus Core sends to plugins, so that I can understand data shapes, field values, and request structures without building a full plugin first.
...
1. **Given** the recorder plugin is configured with an output directory, **When** `finfocus cost projected` is executed with a Pulumi plan, **Then** each request is saved as a separate JSON file with timestamp, method name, and unique identifier in the filename.
...
3. **Given** the recorder is built and installed, **When** `finfocus plugin validate` is run, **Then** the recorder passes all validation checks as a conformant plugin.
...
- **FR-004**: System MUST support configuring the output directory via `FINFOCUS_RECORDER_OUTPUT_DIR` environment variable (default: `./recorded_data`).
- **FR-005**: System MUST support enabling/disabling mock responses via `FINFOCUS_RECORDER_MOCK_RESPONSE` environment variable (default: `false`).
...
- **FR-008**: System MUST use the pluginsdk library from finfocus-spec v0.4.6 or later for SDK patterns (BasePlugin, Matcher, Serve, request validation helpers).
...
- **FR-013**: System MUST follow the standard plugin binary naming convention: `finfocus-plugin-recorder`.
...
- **SC-003**: The recorder plugin passes all Core plugin validation checks (`finfocus plugin validate`).
- **SC-004**: Recorded JSON files are human-readable and contain all fields present in the original gRPC requests.
- **SC-005**: Mock responses are valid according to the finfocus-spec protocol definitions (can be deserialized without errors).
...
- **finfocus-spec v0.4.6+**: Required for pluginsdk with request validation helpers
  - Release: https://github.com/rshade/finfocus-spec/releases/tag/v0.4.6
...
- The finfocus-spec SDK (`pluginsdk`) v0.4.6+ provides all necessary building blocks (BasePlugin, Matcher, Serve, builders, validation helpers).
- The recorder will be shipped as part of finfocus-core releases or as an easily buildable target.
