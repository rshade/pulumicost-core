...
- Q: How should the tool detect it is running in plugin mode (especially on Windows)? → A: **Hybrid Approach**: Detect if binary name matches `pulumi-tool-cost` (case-insensitive, ignoring `.exe`) OR if `PULUMICOST_PLUGIN_MODE=true` is set.
- Q: Where specifically should configuration be stored when PULUMI_HOME is used? → A: **Subdirectory**: Store config in `$PULUMI_HOME/pulumicost/`.
- Q: What should happen if the tool is in plugin mode but `PULUMI_HOME` is missing? → A: **Silent Fallback**: Use default standalone configuration paths (e.g., XDG or `~/.pulumicost`) while maintaining plugin-style help text.
...
- Q: What version should the plugin report? → A: **Sync with Core**: The plugin version MUST match the current `pulumicost` core version.

## User Scenarios & Testing

### User Story 1 - Invoke via Pulumi CLI (Priority: P1)

As a Pulumi user, I want to execute `pulumicost` commands using `pulumi plugin run tool cost` so that I can integrate cost estimation directly into my Pulumi workflow without needing a separate binary in my path.
...
As a user, when I run the tool as a plugin, I want the help text and usage examples to reflect the `pulumi plugin run tool cost` syntax instead of `pulumicost` so that I can copy-paste examples correctly.
...
2. **Given** I am running the binary directly as `pulumicost`, **When** I run help, **Then** the usage line remains `Use: "pulumicost"`.
...
2. **Given** `PULUMI_HOME` is NOT set, **When** the tool runs, **Then** it falls back to standard XDG paths or `~/.pulumicost`.

### Edge Cases

- **Missing PULUMI_HOME**: If the tool is in plugin mode but `PULUMI_HOME` is not set, it MUST silently fallback to default standalone configuration paths (XDG or `~/.pulumicost`) but keep using plugin-style help text (`pulumi plugin run tool cost`).
...
- **FR-001**: The application MUST detect its invocation mode via **EITHER**:
    ...
    - The environment variable `PULUMICOST_PLUGIN_MODE` is set to `true` (or `1`).
...
- **FR-005**: The application MUST strictly respect `PULUMICOST_` prefix for its own specific environment variables (e.g. API keys for pricing providers) to avoid collision with Pulumi core variables.
- **FR-006**: When in plugin mode, the application MUST prioritize `$PULUMI_HOME/pulumicost/` as the configuration directory if `$PULUMI_HOME` is set.
...
- **SC-001**: `pulumi plugin ls` successfully lists `cost` as a tool plugin with a version matching the `pulumicost` core binary.
...
- **SC-004**: The binary functions identically to the standalone `pulumicost` regarding core logic (cost calculation), passing all existing regression tests.