...
| `PluginMode` | `boolean` | Whether the app is running as a plugin | Binary Name or Env: `PULUMICOST_PLUGIN_MODE` |
...
if PluginMode AND PulumiHome != "":
    return Join(PulumiHome, "pulumicost")
else:
...
These variables are available to the process when run by Pulumi, though currently `pulumicost` only actively uses `PULUMI_HOME`.

| Env Variable | Description | Usage in Feature |
|--------------|-------------|------------------|
| `PULUMI_HOME` | Pulumi config root | Used to anchor `pulumicost` config |
