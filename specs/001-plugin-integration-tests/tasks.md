...
**Feature**: `001-plugin-integration-tests`
**Status**: Complete
**Spec**: [specs/001-plugin-integration-tests/spec.md](spec.md)
...
- [ ] T001 [P] Create `test/integration/plugin/setup_test.go` with mock registry server implementation (httptest) serving JSON metadata and binary artifacts
- [ ] T002 [P] Implement `MockRelease` and `MockAsset` structs in `test/integration/plugin/setup_test.go` matching GitHub API format
- [ ] T003 [P] Add helper function `StartMockRegistry(t *testing.T)` returning `*httptest.Server` and cleanup function
- [ ] T004 [P] Add helper function `CreateTestPluginArchive(t *testing.T, name, version, os, arch string)` that generates a valid .tar.gz/.zip plugin artifact
- [ ] T005 [P] Add `Makefile` target `test-integration-plugin` to run only these new tests
...
- [ ] T007 [P] Create `test/integration/plugin/concurrency_test.go` to test concurrent install operations (validating T006)
...
- [ ] T008 [P] [US1] Create `test/integration/plugin/init_test.go`
- [ ] T009 [P] [US1] Implement `TestPluginInit_Basic` verifying directory creation, `main.go`, `go.mod`, and `manifest.yaml` content
- [ ] T010 [P] [US1] Implement `TestPluginInit_MultiProvider` verifying manifest contains multiple providers
- [ ] T011 [P] [US1] Implement `TestPluginInit_CustomOutputDir` verifying creation in specified path
- [ ] T012 [P] [US1] Implement `TestPluginInit_Force` verifying overwrite behavior
- [ ] T013 [P] [US1] Implement `TestPluginInit_InvalidName` verifying error handling
...
- [ ] T014 [P] [US2] Create `test/integration/plugin/install_test.go`
- [ ] T015 [P] [US2] Implement `TestPluginInstall_FromRegistry` using mock registry
- [ ] T016 [P] [US2] Implement `TestPluginInstall_SpecificVersion` verifying correct version download
- [ ] T017 [P] [US2] Implement `TestPluginInstall_FromURL` verifying direct URL install and security warning log
- [ ] T018 [P] [US2] Implement `TestPluginInstall_Force` verifying reinstall behavior
- [ ] T019 [P] [US2] Implement `TestPluginInstall_NoSave` verifying config is not modified
...
- [ ] T020 [P] [US3] Create `test/integration/plugin/update_test.go`
- [ ] T021 [P] [US3] Implement `TestPluginUpdate_ToLatest` (requires installing v1 then updating to v2 via mock registry)
- [ ] T022 [P] [US3] Implement `TestPluginUpdate_SpecificVersion` verifying update to requested version
- [ ] T023 [P] [US3] Implement `TestPluginUpdate_DryRun` verifying no changes are made
- [ ] T024 [P] [US3] Implement `TestPluginUpdate_AlreadyUpToDate` verifying correct message
- [ ] T025 [P] [US3] Implement `TestPluginUpdate_NonExistent` verifying error handling
...
- [ ] T026 [P] [US4] Create `test/integration/plugin/remove_test.go`
- [ ] T027 [P] [US4] Implement `TestPluginRemove_Basic` verifying file and config removal
- [ ] T028 [P] [US4] Implement `TestPluginRemove_KeepConfig` verifying config retention
- [ ] T029 [P] [US4] Implement `TestPluginRemove_Aliases` verifying `uninstall` and `rm` work
- [ ] T030 [P] [US4] Implement `TestPluginRemove_NonExistent` verifying error handling

## Phase 7: Polish & Cleanup

**Goal**: Ensure code quality and documentation.

- [ ] T031 Run `make lint` and fix any linting errors in new test files
- [ ] T032 Verify all tests pass with `go test -v ./test/integration/plugin/...`
- [ ] T033 Update `test/integration/README.md` (if exists) or create one describing how to run these tests
