...
func TestE2E_ProjectedCostWorkflow(t *testing.T) {
	...
	cmd := exec.CommandContext(ctx, binaryPath, "cost", "projected", "--pulumi-json", planPath, "--output", "json")
...
func TestE2E_ProjectedCostWorkflow_TableOutput(t *testing.T) {
	...
	cmd := exec.CommandContext(ctx, binaryPath, "cost", "projected", "--pulumi-json", planPath, "--output", "table")
	output, err := cmd.Output()
	require.NoError(t, err, "Command failed: %v", err)
...
func TestE2E_ActualCostWorkflow(t *testing.T) {
	...
	cmd := exec.CommandContext(ctx, binaryPath, "cost", "actual", "--from", "2024-01-01", "--to", "2024-01-31", "--output", "json")
	output, err := cmd.Output()
...
func TestE2E_InvalidInputHandling(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args: []string{"cost", "projected", "--pulumi-json", "/dev/null", "--output", "invalid"},
			expectError: true,
		},
	}
...
func TestE2E_MultipleOutputFormats(t *testing.T) {
	...
	for _, format := range formats {
		t.Run(format+"_format", func(t *testing.T) {
			...
			output, err := cmd.Output()
			require.NoError(t, err, "Command failed for format %s: %v", format, err)
...
// Helper function to build the pulumicost binary for testing
func buildPulumicostBinary(t *testing.T) string {
	// Create a temporary binary path
	tempDir := t.TempDir()
	binaryPath := filepath.Join(tempDir, "pulumicost")
