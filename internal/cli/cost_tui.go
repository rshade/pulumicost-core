...
func RenderCostOutput(
	...
	// 1. If output format is explicitly structured (JSON/NDJSON), bypass TUI completely.
...
func renderStyledOutput(w io.Writer, resultWithErrors *engine.CostResultWithErrors) error {
	...
	// Also display errors if any
	if resultWithErrors.HasErrors() {
		fmt.Fprintln(w)
		fmt.Fprint(w, resultWithErrors.ErrorSummary()) // TODO: Styled error summary?
	}
